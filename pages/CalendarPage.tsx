            ...
            <div className="flex-1 relative border-l-2 border-foreground/10 h-[1440px]">
                ...
                {positionedEvents.map(event => {
                    ...
                    return (
                        <div
                            key={event.id}
                            className="absolute p-1 rounded-lg cursor-pointer flex flex-col shadow-lg"
                            style={{
                                top: `${event.top}px`,
                        ...
                        >
                            <strong className="text-xs font-semibold leading-tight">{event.title}</strong>
                            <span className="text-[11px] opacity-90 leading-tight">
                                {new Date(event.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - {new Date(event.endTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            </span>
                            {isClass ? (
                                location && <p className="text-[11px] opacity-80 leading-tight">{location}</p>
                            ) : (
                                event.description && <p className="text-[11px] opacity-80 whitespace-pre-wrap leading-tight">{event.description}</p>
                            )}
                        </div>
                    );
                })}
